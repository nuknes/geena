...
- Matrix rain in a floating, semi-transparent foot fullscreen terminal window
- Colors that shift with your Caelestia dynamic scheme (Material You extraction)
- Auto-triggers on idle, auto-closes on activity
...
Caelestia's default Hyprland rules force foot windows to be fully opaque (`opaque on, opacity 1 1 1`). This means our `opacity = "0.75 override"` on the unimatrix window rule would be ignored as the window stays solid regardless. 
...
## Step 5: Configure the Idle Timeout

Edit `~/.config/quickshell/caelestia/shell.json`. Add a timeout entry to the `idle.timeouts` array:

```json
{
    "timeout": 120,
    "idleAction": ["sh", "-c", "hyprctl clients | grep -q 'unimatrix-screen' || setsid foot --app-id=unimatrix-screen sh -c 'cat ~/.local/state/caelestia/sequences.txt; exec unimatrix' >/dev/null 2>&1 &"],
...
| Part | Purpose |
|------|---------|
| `hyprctl clients \| grep -q 'unimatrix-screen'` | Guard, don't launch a second instance if one is already running |
| `setsid foot --app-id=unimatrix-screen` | Launch a detached foot window with a unique app-id for window rule matching |
| `cat ~/.local/state/caelestia/sequences.txt` | **The magic** injects ANSI escape sequences that re-theme the terminal's 16-color palette to match your active Caelestia scheme before unimatrix starts |
| `exec unimatrix` | Replace the shell with unimatrix (clean process tree) |
| `>/dev/null 2>&1` | Suppress foot's own startup stderr; the terminal's pty (where unimatrix draws) is unaffected |
| `returnAction: pkill` | Kills the terminal when the user returns from idle |

**Why `sequences.txt`?** Caelestia regenerates this file every time your wallpaper or scheme changes. It contains SGR escape codes that remap `term0`–`term15`. By `cat`-ing it into the terminal *before* unimatrix runs, the rain inherits your wallpaper's accent colors instead of the default green.

## Step 6: Tune Your Idle Timeline

A sensible idle sequence (adjust to taste):
...
Timeline: **120s** → rain starts → **300s** → lock (rain visible for ~180s) → **360s** → screen off.
...
│                                                             │
│  shell.json idle timeouts                                   │
│       │                                                     │
│       ├── 120s: launch foot (app-id=unimatrix-screen)       │
│       │         └── cat sequences.txt → theme terminal      │
│       │         └── exec unimatrix → draw rain              │
│       │                                                     │
│       ├── 300s: lock (hyprlock covers the terminal)         │
│       │                                                     │
│       └── 360s: dpms off                                    │
│                                                             │
│  returnAction: pkill foot → terminal closes, rain gone      │
├─────────────────────────────────────────────────────────────┤
...
# Remove the timeout entry from shell.json
